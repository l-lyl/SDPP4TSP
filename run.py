...
from data.temporal_sets_data_loader1 import get_temporal_sets_data_loader
from learner.nsr_learner import NSRLearner
...
def temporal_sets_prediction(parser: argparse.ArgumentParser):
    ...
    train_dl, valid_dl, test_dl = get_temporal_sets_data_loader(
        ...
        pre_kernel_path=config["pre_kernel_path"])
    print(train_dl)
    ...
    learner = NSRLearner(model_cls=model_cls, train_dl=train_dl, valid_dl=valid_dl, test_dl=test_dl, config=config, cuda=args.cuda)
